...
function finishDisplay() {
	// audioRecorder.exportWAV(showWavesufer, 'audio/wav')
}

function showWavesufer(blob) {
	// $('#displayTime').html(
	// 	"<span class='min' id='progressMin'>00</span><span class='colon' id='progressColon'>:</span><span class='sec' id='progressSec'>00.00</span>",
	// )
...
function startRecording(onRecordUpdate, enableFastSample, useEPD, stopOnEPD, useOTF, cbOnStop) {
...
function gotStream(stream) {
	...
	audioRecorder = new CJRecorder(inputPoint)
...
function processEPD(framebuf) {
	...
	while (roundbuffer.length >= 1600) {
		...
		if (usePowerEPD) {
			...
			ret = ChunjaeSTT_Edu_ENG_Recognition_Frame(arr, 800, false)
			if (ret > 0) {
				//audioStopWhenEPD 처리
				if (audioStopWhenEPD) {
					audioRecorder.stop()
			...
			ret = ChunjaeSTT_Edu_ENG_OTF_RecogFrames(arr, 800, 0, false)
			if (ret == 12) {
				audioRecorder.stop()
				while (ret == 12) {
					ret = ChunjaeSTT_Edu_ENG_OTF_RecogFrames(null, 0, 0, false)
				}
				audioRecorder.record()
			}
			if (ret == 13) {
...
function initAudio(onSucceed, onfail) {
...
var _MW_adjustVolume = function (buf) {
	...
	for (var i = 0; i < buf.byteLength - (800 << 1); i += 800 << 1) {
		var fave = 0
		var fvol = 0
		for (var j = i; j < i + (800 << 1); j += 2) fave += viewbuf.getInt16(j, true)
		fave /= 800
		for (var j = i; j < i + (800 << 1); j += 2) fvol += Math.abs(viewbuf.getInt16(j, true) - fave)
